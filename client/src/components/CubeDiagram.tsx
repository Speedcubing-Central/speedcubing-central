...
// The 24-element whole-cube rotation group, generated by x/x'/x2/y/y'/y2/
// z/z'/z2, as a transition table: ROTATION_TRANSITIONS[state][token] = the
// new state after applying `token` from `state`, where each state is keyed
// by its own shortest representative token sequence ('' for identity).
// Needed because two rotations on DIFFERENT axes can still compose to a
// shorter (or empty) net rotation — e.g. "x2 y z y' x" is a genuine no-op —
// which plain same-letter-adjacent cancellation (see simplifyAlg) can never
// catch, since no two of those five tokens share a face letter. Generated
// by BFS over this table and verified against cubing.js's own pattern
// algebra (applying each candidate sequence to a solved 3x3x3 and checking
// isIdentical), not hand-derived — see the FACE_RELABEL tables below for
// the same verified-not-derived standard applied elsewhere in this file.
...
// Reduces an arbitrary run of PURE rotation tokens (only x/y/z family — no
// face turns mixed in) to its shortest equivalent, possibly empty (a net
// no-op) — e.g. ['x2','y','z',"y'",'x'] -> [].
...
//
// A second pass then collapses every maximal run of pure rotation tokens
// (see reduceRotations above) to its shortest equivalent — same-letter
// cancellation alone can't catch a run like "x2 y z y' x" (a genuine
// no-op) since no two adjacent tokens there share a face letter. Safe
// anywhere such a run occurs, not just at the very start/end: rotations
// with nothing but other rotations between them always compose
// associatively, so replacing the run changes nothing else about the alg.
