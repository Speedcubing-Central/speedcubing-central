...
export function Layout({ children }: { children: ReactNode }) {
  ...
  // site. It builds its own header (logo + sign-in links), so this skips
  // the sidebar, mobile top bar, and mobile bottom bar entirely rather than
  // just hiding them, the way focus mode does for logged-in pages.
        ...
        <nav className="flex-1 flex flex-col gap-1 overflow-y-auto">
          {visible.map((n) => (
            <NavLink
              ...
              className={({ isActive }) =>
                clsx(
                  'flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50',
                  isActive
