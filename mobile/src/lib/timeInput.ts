...
// Verbatim port of client/src/lib/timeInput.ts, manual time entry must
// interpret a typed string identically on both platforms (e.g. at precision 2,
// "1258" is 12.58s and "12684" is 1:26.84, and a trailing "+" stacks a +2).
//
...
export function parseTimeInput(
  raw: string,
  precision: number,
): { time: number; penalty: Penalty; plusTwoCount: number } | null {
  const t = raw.trim();
  if (!t) return null;
