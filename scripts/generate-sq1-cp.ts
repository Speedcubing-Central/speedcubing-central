// Regenerate client/src/data/sq1CP.ts and
// client/src/data/sq1CPSetupAlgs.generated.ts after re-scraping
// scripts/scdb-raw/sq1cp.txt by running, from the repo root:
//
//   node --experimental-strip-types scripts/generate-sq1-cp.ts
//
// scripts/scdb-raw/sq1cp.txt is a scrape of speedcubedb.com's Square-1
// "Corner Permutation" listing page (https://speedcubedb.com/a/SQ1/SQ1CP),
...
// shape-only check: a Corner Permutation case's setup only disturbs corner
// permutation (Cube Shape and edge state are unaffected), so a correct CP
...
// CP setup+algorithm pairs (and for CO — see generate-sq1-co.ts, the first
...
const RAW_FILE = new URL('./scdb-raw/sq1cp.txt', import.meta.url);
...
async function main() {
  ...
  fs.writeFileSync(new URL('../client/src/data/sq1CP.ts', import.meta.url), setHeader);
  console.error('Wrote client/src/data/sq1CP.ts with', outCases.length, 'cases');

  const setupHeader = [
    '// GENERATED FILE, do not edit by hand.',
    '// Regenerate with: node --experimental-strip-types scripts/generate-sq1-cp.ts',
    'export const SQ1_CP_SETUP_ALGS: Record<string, string> = '
      + JSON.stringify(setupAlgs, null, 2) + ';',
    '',
  ].join('\n');
  fs.writeFileSync(new URL('../client/src/data/sq1CPSetupAlgs.generated.ts', import.meta.url), setupHeader);
  console.error('Wrote client/src/data/sq1CPSetupAlgs.generated.ts');
}
