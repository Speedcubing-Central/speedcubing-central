// Regenerate client/src/data/sq1EO.ts and
// client/src/data/sq1EOSetupAlgs.generated.ts after re-scraping
// scripts/scdb-raw/sq1eo.txt by running, from the repo root:
//
//   node --experimental-strip-types scripts/generate-sq1-eo.ts
//
// scripts/scdb-raw/sq1eo.txt is a scrape of speedcubedb.com's Square-1
// "Edge Orientation" listing page (https://speedcubedb.com/a/SQ1/SQ1EO),
...
// shape-only check: an Edge Orientation case's setup only disturbs edge
// orientation (Cube Shape and corner state are unaffected), so a correct EO
...
// EO setup+algorithm pairs (and for CO — see generate-sq1-co.ts, the first
...
const RAW_FILE = new URL('./scdb-raw/sq1eo.txt', import.meta.url);
...
async function main() {
  ...
  fs.writeFileSync(new URL('../client/src/data/sq1EO.ts', import.meta.url), setHeader);
  console.error('Wrote client/src/data/sq1EO.ts with', outCases.length, 'cases');

  const setupHeader = [
    '// GENERATED FILE, do not edit by hand.',
    '// Regenerate with: node --experimental-strip-types scripts/generate-sq1-eo.ts',
    'export const SQ1_EO_SETUP_ALGS: Record<string, string> = '
      + JSON.stringify(setupAlgs, null, 2) + ';',
    '',
  ].join('\n');
  fs.writeFileSync(new URL('../client/src/data/sq1EOSetupAlgs.generated.ts', import.meta.url), setupHeader);
  console.error('Wrote client/src/data/sq1EOSetupAlgs.generated.ts');
}
